...
const LOCAL_SECTOR_SIDE_LENGTH_LY = 125;
...
const PROXIMA_BINARY_ORBIT_FACTOR = 0.0175;
const PROXIMA_STAR3_ORBIT_FACTOR = 0.37;
const PROXIMA_STAR3_PERIOD_SECONDS = 44;
const PROXIMA_BINARY_PERIOD_SECONDS = 1;
const PROXIMA_OUTER_PLANET_PERIOD_SECONDS = 9;
...
const v1ShipDataFallback = {
  ...
  Starship: {
    ...
    description:
      "An enormous mobile base with an erosion shield, habitation, and carrier capabilities, designed to traverse distances between stars at relativistic speeds over the course of decades.",
  },
};
...
function localSectorShipSpeedMultiplier() {
...
function updateLocalSectorClock(dt, size) {
  ...
  const shipSpeedMultiplier = localSectorShipSpeedMultiplier();
  if (shipSpeedMultiplier <= 0) return;

  const nominalShipSpeedPxPerSec = size * LOCAL_SECTOR_BASE_SHIP_SPEED_FACTOR * shipSpeedMultiplier;
  if (nominalShipSpeedPxPerSec <= 0) return;
...
function tick(frameTs) {
  ...
  if (currentScreen === "0") {
    ...
    updateLocalSectorClock(dt, starCanvas.width);
    updateLocalSectorTimeLabel();
    updateStarships(dt);
    drawStarScene();
